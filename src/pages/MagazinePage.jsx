...
const STYLES = `
  ...
  .page-l {
    background: var(--bw-ink); border-right: 1px solid var(--bw-900);
    border-radius: 8px 0 0 8px; padding: 52px 44px;
    position: relative; overflow: hidden;
  }
  .page-r {
    background: var(--bw-ink); border-radius: 0 8px 8px 0;
  ...
  .pgnum { font-family: 'JetBrains Mono',monospace; font-size: 10px; color: var(--bw-800); letter-spacing: 0.12em; margin-bottom: 20px; }
  ...
  .art-sub { font-size: 12px; color: var(--bw-600); line-height: 1.6; margin: 0 0 16px; font-style: italic; }
  .divider { width: 32px; height: 2px; border-radius: 1px; margin-bottom: 16px; }
  .art-body { font-size: 13px; color: var(--bw-400); line-height: 1.9; overflow: hidden; display: -webkit-box; -webkit-line-clamp: 14; -webkit-box-orient: vertical; flex: 1; }
  .ins-box { border-top: 1px solid var(--bw-900); padding-top: 14px; margin-top: auto; }
  ...
  .ins-text { font-size: 12px; color: var(--bw-400); line-height: 1.6; font-style: italic; }
      ...
      <div style={{ height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
        ...
        <div>
          ...
          <h1 style={{ fontFamily: 'var(--f-serif)', fontSize: 'clamp(30px,5vw,48px)', fontWeight: 900, lineHeight: 1.05, color: 'var(--bw-white)', margin: '0 0 18px', letterSpacing: '-0.02em' }}>
            창업의<br /><span style={{ color: 'var(--bw-white)' }}>모든 것</span>
          </h1>
        ...
        <div>
          <div style={{ width: '40px', height: '2px', background: 'var(--bw-white)', borderRadius: '1px', marginBottom: '20px' }} />
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
            {Array.from({ length: total }).map((_, i) => (
              <div key={i} onClick={() => { setSpread(i); setKey(k => k + 1) }}
                style={{ width: i === spread ? '16px' : '4px', height: '4px', borderRadius: i === spread ? '2px' : '50%', background: i === spread ? 'var(--bw-white)' : '#1e1e1e', cursor: 'pointer', transition: 'all 0.3s' }} />
            ))}
