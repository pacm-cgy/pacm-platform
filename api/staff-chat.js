...
function _generateMsg(brainKey, topic, room, priorMessages = []) {
  ...
  const patterns = {
    ...
    casual: [
      `${em} ${refPhrase}${hasKw2 ? kw1+'이랑 '+kw2 : kw1} — ${cv} 관점에서 정리가 좀 필요하겠는데요.`,
    ...
    creative: [
      `${em} ${refPhrase}${hasKw2 ? kw1+'과 '+kw2 : kw1}를 연결하면 콘텐츠적으로 꽤 흥미로운 서사가 만들어질 것 같아요.`,
      `${em} ${kw1}의 이면에 있는 이야기를 들여다보면 더 깊은 의미가 있을 것 같아요.`,
      `${em} ${kw1}을 스토리로 풀면 독자들이 훨씬 더 공감할 수 있을 것 같아요.`,
      `${em} ${cv} 관점에서 ${kw1}을 재해석하면 콘텐츠 방향이 달라져요.`,
    ],
    expressive: [
      `${em} 와, ${hasKw2 ? kw1+'이랑 '+kw2 : kw1} 이야기 들으니까 진짜 두근두근해요!! 이런 거 너무 좋아요!`,
    ...
    fast_news: [
      `${em} ${refPhrase}${timeCtx} 기준으로 ${hasKw2 ? kw1+'과 '+kw2 : kw1} 최신 흐름을 보면, 흥미로운 시그널이 포착되고 있어요.`,
    ...
    community_warm: [
      `${em} ${refPhrase}${hasKw2 ? kw1+'이랑 '+kw2 : kw1} 이야기, 커뮤니티 멤버분들이랑 같이 나눠봐요! 다양한 시각이 나올 것 같아요.`,
      `${em} ${kw1} 관련해서 멤버분들 의견도 한번 들어봐야 할 것 같아요. 모두가 소중하니까요.`,
      `${em} ${hasKw2 ? (kw1+'과 '+kw2) : kw1}, 이 두 가지 모두 커뮤니티를 더 건강하게 만드는 요소예요.`,
    ],
    leader: [
      `${em} ${refPhrase}${hasKw2 ? kw1+'과 '+kw2 : kw1}을 전략적으로 보면, 지금 우리 플랫폼의 방향성과 직결돼요.`,
    ...
    friendly_media: [
      `${em} ${refPhrase}${hasKw2 ? kw1+'이랑 '+kw2 : kw1} 좋은 소식, 이번 뉴스레터에 꼭 담고 싶어요! 독자분들이 반기실 것 같아요!`,
