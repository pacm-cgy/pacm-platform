...
// ARIA의 고유 username으로 프로필을 찾고, 없으면 admin fallback
...
async function createMonthlyEvent(adminId, stats) {
  ...
  try {
    const check = await fetch(
      `${SB_URL}/rest/v1/events?created_at=gte.${monthStart}&type=eq.challenge&limit=1&select=id`,
      { headers: H() }
    )
  ...
  const MONTHLY_EVENTS = [
    {
      ...
      description: `Insightship AI가 준비한 ${month}월 창업 아이디어 챌린지입니다!\n\n**주제**: "${hot}" 분야에서 사회 문제를 해결하는 창업 아이디어\n\n**참가 방법**\n1. 아이디어랩에 아이디어 게시\n2. 커뮤니티에 공유\n3. AI 멘토로 피드백 받기\n\n**기간**: ${month}월 내내\n\n참가비 무료, 누구나 참여 가능!`,
      type: 'challenge',
      status: 'upcoming',
