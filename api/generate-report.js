...
export default async function handler(req) {
  ...
  // ── 리포트 1+2 병렬 생성 (Edge 25초 이내) ──────────────────
  await Promise.all([
  // 리포트 1: 투자·자금 동향
  (async () => { try {
    const slug1 = `ai-funding-report-${label.replace(/[^0-9월주]/g, '-').replace(/-+/g, '-').replace(/-$/, '')}`
...
전체 3000~4000자, 각 섹션 500~700자 기준 엄수. ## 헤더로 시작.`
  ...
  // 리포트 2: 시장·생태계 동향
  (async () => { try {
    const slug2 = `ai-market-report-${label.replace(/[^0-9월주]/g, '-').replace(/-+/g, '-').replace(/-$/, '')}`
