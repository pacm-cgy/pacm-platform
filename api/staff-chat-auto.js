...
async function handleAdminMessage(room, adminMessage, adminSenderName = '관리자') {
  ...
  for (let i = 0; i < responders.length; i++) {
    ...
    const baseDelay = i === 0
      ? (mood.key === 'enthusiastic' ? 180 : mood.key === 'tired' ? 900 : mood.key === 'stressed' ? 300 : 420)
      : (mood.key === 'enthusiastic' ? 450 : mood.key === 'tired' ? 1300 : 720)
    const jitter = Math.random() * (mood.key === 'focused' ? 250 : 650)
